    ...
    const ShowAllUsers = async () =>
    {
      SetUsers([]);
        ...
        var GetUsers = allUsers.data.map(async(obj, key) =>
        {
          ...
          var userInfo = await getUserInfoById(UsersUrl,obj._id); 
          ...
          User.SessionTimeOut = "No time limit";
          ...
          if(userInfo.data[0][0].Premissions.length >= 1)
          {
            ...
            userInfo.data[0][0].Premissions.forEach(element => {
              if(JSON.stringify(element).includes("true"))  //check if element is true
              {
                counter++;
                var String = JSON.stringify(element);
                String = String.replace('_',' ').replace('}','').replace('{','').replace('"','').replace(':','').replace('true','').replace('"','');
        ...
        GetUsers[allUsers.data.length - 1].then((result) => {
